...
void substrate::largeFreer::operator()(void *ptr) const {
  largeFree(ptr, bytes);
}
...
LAptr substrate::largeMallocInterleaved(size_t bytes, unsigned numThreads) {
  ...
  return LAptr{data, largeFreer{bytes}};
}

LAptr substrate::largeMallocLocal(size_t bytes) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a prefaulted allocation
  return LAptr{allocPages(bytes / allocSize(), true), largeFreer{bytes}};
}

LAptr substrate::largeMallocFloating(size_t bytes) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a non-prefaulted allocation
  return LAptr{allocPages(bytes / allocSize(), false), largeFreer{bytes}};
}

LAptr substrate::largeMallocBlocked(size_t bytes, unsigned numThreads) {
  // round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  // Get a non-prefaulted allocation
  void *data = allocPages(bytes / allocSize(), false);
 ...
 */
template <typename RangeArrayTy>
LAptr substrate::largeMallocSpecified(size_t bytes, uint32_t numThreads,
...
// file
template LAptr substrate::largeMallocSpecified<std::vector<uint32_t>>(
    size_t bytes, uint32_t numThreads, std::vector<uint32_t> &threadRanges,
    size_t elementSize);
template LAptr substrate::largeMallocSpecified<std::vector<uint64_t>>(
