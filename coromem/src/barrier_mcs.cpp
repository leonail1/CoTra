...
class MCSBarrier : public substrate::Barrier {
  struct treenode {
  ...
  std::vector<substrate::CacheLineStorage<treenode>> nodes;
...
public:
  ...
  virtual void wait() {
    treenode &n = nodes.at(ThreadPool::getTID()).get();
...
std::unique_ptr<substrate::Barrier>
substrate::createMCSBarrier(unsigned activeThreads) {
