#ifndef COROGRAPH_MEM_H
#define COROGRAPH_MEM_H
...
template <class SourceHeap> class ThreadPrivateHeap {
...
template <class SourceHeap> class LockedHeap : public SourceHeap {
...
template <typename SourceHeap> class ZeroOut : public SourceHeap {
public:
  enum { AllocSize = SourceHeap::AllocSize };
...
//! Maintain a freelist
template <class SourceHeap> class FreeListHeap : public SourceHeap {
  struct FreeNode {
    FreeNode *next;
  };
  FreeNode *head;
...
public:
  ...
  void clear() {
    while (head) {
      printf("clear can't end... it is error\n");
      printf("author only use allocate and deallocate to add or delete element "
             "from linked list.\n");
      FreeNode *N = head;
      head = N->next;
  ...
  inline void *allocate(size_t size) {
    if (head) {
      ...
      // printf("I suppose it never come here..\n");
      return ptr;
    } else {
...
template <class SourceHeap> class SelfLockFreeListHeap : public SourceHeap {
  struct FreeNode {
    FreeNode *next;
  };
  FreeNode *head;
...
public:
  ...
  inline void *allocate(size_t COROMEM_USED_ONLY_IN_DEBUG(size)) {
...
template <typename SourceHeap> class BumpHeap : public SourceHeap {
  struct Block {
    union {
      Block *next;
 ...
 */
template <typename SourceHeap> class BumpWithMallocHeap : public SourceHeap {
  struct Block {
    union {
      Block *next;
      double dummy; // for alignment
...
#ifdef COROMEM_FORCE_STANDALONE
...
template <typename Ty> class FixedSizeAllocator;

template <> class FixedSizeAllocator<void> {
public:
...
public:
  ...
  void deallocate(pointer ptr, size_type COROMEM_USED_ONLY_IN_DEBUG(len)) {
    assert(len == 1);
    heap.deallocate(ptr);
  }
...
template <typename Ty> class Pow_2_BlockAllocator {

  template <typename T> static inline void destruct(T *t) {
...
template <> class Pow_2_BlockAllocator<void> {
public:
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
  typedef void *pointer;
...
template <typename Ty, typename HeapTy> class ExternalHeapAllocator;

template <typename HeapTy> class ExternalHeapAllocator<void, HeapTy> {
public:
  typedef size_t size_type;
  typedef ptrdiff_t difference_type;
  typedef void *pointer;
