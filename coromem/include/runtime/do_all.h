#ifndef COROGRAPH_EXECUTOR_DOALL_H
#define COROGRAPH_EXECUTOR_DOALL_H
...
template <typename R, typename F, typename ArgsTuple> class DoAllStealingExec {
  ...
  struct ThreadContext {
    ...
    alignas(substrate::COROMEM_CACHE_LINE_SIZE) substrate::SimpleLock work_mutex;
    unsigned id;
    Iter shared_beg;
    Iter shared_end;
    Diff_ty m_size;
    ...
    ThreadContext()
        : work_mutex(), id(getThreadPool().getMaxThreads()), shared_beg(),
          shared_end(), m_size(0) {}
...
private:
  COROMEM_ATTRIBUTE_NOINLINE bool
  ...
  COROMEM_ATTRIBUTE_NOINLINE bool stealOutsideSocket(ThreadContext &poor,
                                                    const StealAmt &amt) {
    bool sawWork = false;
    bool stoleWork = false;
    auto &tp = getThreadPool();
...
public:
  DoAllStealingExec(const R &_range, F _func, const ArgsTuple &argsTuple)
      : range(_range), func(_func), loopname(getLoopName(argsTuple)),
...
template <bool _STEAL> struct ChooseDoAllImpl {
  ...
  static void call(const R &range, F &&func, const ArgsT &argsTuple) {
    ...
    getThreadPool().run(activeThreads, [&exec](void) { exec.initThread(); },
  ...
  static void call(const R &range, F func, const ArgsT &argsTuple) {
    ...
    runtime::on_each_gen(
        [&](const unsigned int, const unsigned int) {
          // printf("from thread %d\n", substrate::getThreadPool().getTID());
          static constexpr bool NEED_STATS = NeedStats<ArgsT>::value;
          ...
          if (NEED_STATS) {
            // runtime::reportStat_Tsum(loopname, "Iterations", iter);
          }
        },
        std::make_tuple());
  }
...
void do_all_gen(const R &range, F &&func, const ArgsTuple &argsTuple) {
  ...
  runtime::OperatorReferenceType<decltype(std::forward<F>(func))> func_ref =
      func;
  ChooseDoAllImpl<STEAL>::call(range, func_ref, argsT);
}
