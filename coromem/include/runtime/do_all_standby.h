#ifndef DOALL_STANBY_H
#define DOALL_STANBY_H
...
template <typename R, typename F, typename FS, typename ArgsTuple>
class DoAllStandbyExec {
 ...
 public:
  DoAllStandbyExec(const R &_range, F _func, const ArgsTuple &argsTuple)
      : range(_range),
        func(_func),
  ...
  DoAllStandbyExec(
      const R &_range, F _func, FS _stb_func, const ArgsTuple &argsTuple)
      : range(_range),
        func(_func),
        stb_func(_stb_func),
        loopname(getLoopName(argsTuple)),
...
struct ChooseDoAllStanbyImpl {
  template <typename R, typename F, typename FS, typename ArgsT>
  static void call(
      const R &range, F &&func, FS &&stb_func, const ArgsT &argsTuple) {
    DoAllStandbyExec<
        ...
        ArgsT>
        exec(
            range, std::forward<F>(func), std::forward<FS>(stb_func),
            argsTuple);
...
template <typename R, typename F, typename FS, typename ArgsTuple>
void do_all_standby_gen(
    const R &range, F &&func, FS &&stb_func, const ArgsTuple &argsTuple) {
  ...
  ChooseDoAllStanbyImpl<STEAL>::call(range, func_ref, stb_func_ref, argsT);
}

#endif  // DOALL_STANBY_H
