#ifndef COROGRAPH_OBIM_H
#define COROGRAPH_OBIM_H
...
struct OBIM {
  ...
  struct ThreadData {
    std::map<Index, PQueue *, std::less<Index>> local;
    Index curIndex;
    Index scanStart;
    PQueue *current;
  ...
  PerSocketStorage<ConExtLinkedQueue<Block, true>> blockList;
...
public:
  ...
  void summaryTime() {
    ...
    printf("totalTime: %lu avgTime: %lu\n", totaltime / 1000000,
           totaltime / 1000000 / activeThreads);
  }
