 ...
 */
template <typename T> class LargeArray {
...
protected:
  ...
  void allocate(size_type n, AllocType t) {
    ...
    case Interleaved:
      ...
      m_realdata =
          substrate::largeMallocInterleaved(n * sizeof(T), getActiveThreads());
      break;
    case Local:
...
public:
  ...
  void destroy() {
    ...
    // do_all(iterate(m_data, m_data + m_size), [=](TV &i) { (&i)->~TV(); });
    for(size_t i=0;i<m_size;i++){
       (&m_data[i])->~T();
    }
  }
...
//! Void specialization
template <> class LargeArray<void> {
...
public:
  ...
  template <typename AnyTy> void set(difference_type, AnyTy) {}
  ...
  template <typename... Args> void constructAt(size_type, Args &&...) {}
  template <typename... Args> void create(size_type, Args &&...) {}
