...
#include "../runtime/do_all_double.h"
#include "../runtime/do_all_standby.h"
#include "../runtime/mem.h"
#include "../runtime/on_each.h"
...
void do_all(const RangeFunc &rangeMaker, FunctionTy &&fn, const Args &...args) {
  auto tpl = std::make_tuple(args...);
  do_all_gen(rangeMaker(tpl), std::forward<FunctionTy>(fn), tpl);
}

/**
 * do-all-standby loop. All iterations should be independent.
 * Operator should conform to <code>fn(item)</code> where item is a value from
 * the iteration range.
 *
 * @param rangeMaker an iterate range maker typically returned by
 ...
 */
template <
    typename RangeFunc, typename FunctionTy, typename StandbyFunctionTy,
    typename... Args>
void do_all_standby(
    const RangeFunc &rangeMaker, FunctionTy &&fn, StandbyFunctionTy &&stdby_fn,
    const Args &...args) {
  auto tpl = std::make_tuple(args...);
  do_all_standby_gen(
      rangeMaker(tpl), std::forward<FunctionTy>(fn),
      std::forward<StandbyFunctionTy>(stdby_fn), tpl);
}
 ...
 */
template <
    typename RangeFunc, typename FunctionTy, typename DoubleFunctionTy,
    typename StandbyFunctionTy, typename... Args>
void do_all_double(
    const RangeFunc &rangeMaker, FunctionTy &&fn, DoubleFunctionTy &&double_fn,
    StandbyFunctionTy &&stdby_fn, const Args &...args) {
  auto tpl = std::make_tuple(args...);
  do_all_double_gen(
      rangeMaker(tpl), std::forward<FunctionTy>(fn),
      std::forward<DoubleFunctionTy>(double_fn),
      std::forward<StandbyFunctionTy>(stdby_fn), tpl);
}
...
struct DoAll {
  ...
  void operator()(
      const RangeFunc &rangeMaker, const F &f, Args &&...args) const {
    do_all(rangeMaker, f, std::forward<Args>(args)...);
  }
};
