...
template <typename T> struct gmax {
...
//! gmax is the functional form of std::max
template <typename T> struct gmin {
  constexpr T operator()(const T &lhs, const T &rhs) const {
    return std::min<T>(lhs, rhs);
  }
};

template <typename T, T value> struct identity_value {
...
template <typename T> struct identity_value_zero {
  constexpr T operator()() const { return T{0}; }
};

template <typename T> struct identity_value_min {
  constexpr T operator()() const { return std::numeric_limits<T>::min(); }
};

template <typename T> struct identity_value_max {
