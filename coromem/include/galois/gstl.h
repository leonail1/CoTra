...
class SerFIFO : public SerCont<T, C> {
  using Base = SerCont<T, C>;
...
class SerStack : public SerCont<T, C> {
  using Base = SerCont<T, C>;
...
template <typename IterTy, class Distance>
IterTy safe_advance_dispatch(
    IterTy b, IterTy e, Distance n, std::input_iterator_tag) {
  while (b != e && n--) ++b;
  return b;
}
...
std::pair<IterTy, IterTy> block_range(
    IterTy b, IterTy e, unsigned id, unsigned num) {
...
std::pair<IntTy, IntTy> block_range(
    IntTy b, IntTy e, unsigned id, unsigned num) {
  IntTy dist = e - b;
...
//! Destroy a range
template <typename I>
std::enable_if_t<!std::is_scalar<Val_ty<I>>::value> uninitialized_destroy(
    I first, I last) {
  using T = Val_ty<I>;
...
//! Destroy self
template <typename I>
std::enable_if_t<!std::is_scalar<Val_ty<I>>::value> self_destroy(I first) {
  using T = Val_ty<I>;
  (&*first)->~T();
}

template <class I>
std::enable_if_t<std::is_scalar<Val_ty<I>>::value> uninitialized_destroy(I, I) {
}
