...
#include "../worklists/obim.h"

// Trait classifications

template <typename T> struct trait_has_type { typedef T type; };

template <typename T> struct trait_has_value {
  typedef T type;
  type value;
...
template <typename T, T V> struct trait_has_svalue {
  typedef T type;
...
template <typename T, typename Tuple> constexpr size_t find_trait() {
...
template <typename T, typename Tuple> constexpr bool has_trait() {
 ...
 */
template <typename T, typename Tuple> struct get_trait_type {
  using type = typename std::tuple_element<find_trait<T, Tuple>(),
                                           Tuple>::type; // type = T ?
};
...
template <typename S, typename T, typename D>
constexpr auto get_default_trait_value(
    S COROMEM_UNUSED(source), T COROMEM_UNUSED(tags), D defaults,
    typename std::enable_if<!has_trait<T, S>()>::type * = nullptr) {
...
template <typename S, typename T, typename D>
constexpr auto
get_default_trait_values(std::index_sequence<> COROMEM_UNUSED(seq),
                         S COROMEM_UNUSED(source), T COROMEM_UNUSED(tags),
                         D COROMEM_UNUSED(defaults)) {
  return std::make_tuple();
}

template <size_t... Ints, typename S, typename T, typename D>
constexpr auto
get_default_trait_values(std::index_sequence<Ints...> COROMEM_UNUSED(seq),
...
template <typename> constexpr auto has_function_traits(...) -> bool {
  return false;
}
...
template <typename T, typename... Args> s_wl<T, Args...> wl(Args &&... args) {
 ...
 * ::for_each} loop to be exited deterministically.
 *
 ...
 */
struct det_id_tag {};
template <typename T> struct det_id : public trait_has_value<T>, det_id_tag {
...
template <typename Tup> struct NeedStats {
