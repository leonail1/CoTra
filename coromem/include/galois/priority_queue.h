...
template <typename T, typename Cmp = std::less<T>,
          typename Alloc = FixedSizeAllocator<T>>
...
public:
  ...
  typedef substrate::SimpleLock Lock_ty;

private:
  alignas(substrate::COROMEM_CACHE_LINE_SIZE) Lock_ty mutex;
  Set orderedSet;

public:
  template <typename _T, typename _Cmp = std::less<_T>,
            typename _Alloc = FixedSizeAllocator<_T>>
  using retype =
 ...
 */
template <typename T, typename Cmp = std::less<T>> class ThreadSafeMinHeap {
public:
