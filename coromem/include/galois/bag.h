...
class InsertBag {
 ...
 private:
  ...
  header *newHeaderFromHeap(void *m, unsigned size) {
    ...
    int offset = 1;
    if (sizeof(T) < sizeof(header)) offset += sizeof(header) / sizeof(T);
  ...
  header *newHeader() {
    if (BlockSize) {
      ...
      return newHeaderFromHeap(pagePoolAlloc(), pagePoolSize());
    }
  }
 ...
 public:
  ...
  local_iterator local_begin() {
    return local_iterator(&heads, ThreadPool::getTID());
  }
  local_iterator local_end() {
    return local_iterator(&heads, ThreadPool::getTID() + 1);
  }
  ...
  void pop_front() {
    header *H = heads.getLocal()->first;
    if (H->dbegin == H->dend) {
      throw std::out_of_range("InsertBag::pop_front");
    }
    uninitialized_destroy(H->dbegin, H->dbegin + 1);
    ++H->dbegin;
  }
