#ifndef FIXEDSIZERING_H
#define FIXEDSIZERING_H
...
class FixedSizeRing2 {
 ...
 public:
  ...
  //        return count == 0;
  //    }
  bool full() const { return count == ChunkSize; }
  reference getAt(unsigned x) { return *at(x); }
  const_reference getAt(unsigned x) const { return *at(x); }
  void clear() {
    for (unsigned x = 0; x < count; ++x) datac.destroy(x);
    count = 0;
  }
  template <typename U>
...
class FixedSizeRing {
 ...
 public:
  ...
  void clear() {
    //    assert(precondition());
    for (unsigned x = 0; x < count; ++x) datac.destroy(start + x);
    count = 0;
    start = 0;
  }
