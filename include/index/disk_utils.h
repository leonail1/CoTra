...
#include "diskann/include/windows_customizations.h"
...
DISKANN_DLLEXPORT void add_new_file_to_single_index(
    std::string index_file, std::string new_file);

DISKANN_DLLEXPORT size_t calculate_num_pq_chunks(
    double final_index_ram_limit, size_t points_num, uint32_t dim);

DISKANN_DLLEXPORT void read_idmap(
    const std::string &fname, std::vector<uint32_t> &ivecs);
...
DISKANN_DLLEXPORT int build_merged_vamana_index(
    std::string base_file, diskann::Metric _compareMetric, uint32_t L,
...
DISKANN_DLLEXPORT int build_disk_index(
    ...
    const std::string &codebook_prefix =
        "",  // default is empty for no codebook pass in
    bool use_filters = false,
...
template <typename T, typename LabelT = uint32_t>
DISKANN_DLLEXPORT int build_disk_index_4_merge(
    const char *dataFilePath, const char *indexFilePath,
    const char *indexBuildParameters, diskann::Metric _compareMetric,
    bool use_opq = false,
    const std::string &codebook_prefix =
...
template <typename T, typename LabelT = uint32_t>
DISKANN_DLLEXPORT int kmeans_partition(IndexParameter &index_param);
template <typename T, typename LabelT = uint32_t>
DISKANN_DLLEXPORT int random_partition(IndexParameter &index_param);

template <typename T, typename LabelT = uint32_t>
DISKANN_DLLEXPORT void build_all_partition(IndexParameter &index_param);
template <typename T, typename LabelT = uint32_t>
DISKANN_DLLEXPORT void build_local_partition(IndexParameter &index_param);
DISKANN_DLLEXPORT void collect_replica_partition(IndexParameter &index_param);
DISKANN_DLLEXPORT void merge_replica_partition(IndexParameter &index_param);
DISKANN_DLLEXPORT void delete_temp_file(IndexParameter &index_param);
