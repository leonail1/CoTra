...
static float L2SqrSIMD16ExtResiduals(
    const void *pVect1v, const void *pVect2v, const void *qty_ptr) {
  size_t qty = *((size_t *)qty_ptr);
...
static float L2SqrSIMD4ExtResiduals(
    const void *pVect1v, const void *pVect2v, const void *qty_ptr) {
  size_t qty = *((size_t *)qty_ptr);
  size_t qty4 = qty >> 2 << 2;
...
static int L2SqrI4x(
    const void *__restrict pVect1, const void *__restrict pVect2,
...
class L2SpaceII : public SpaceInterface<int> {
DISTFUNC<int> fstdistfunc_;
size_t data_size_;
size_t dim_;

public:
L2SpaceII(size_t dim) {
  if (dim % 4 == 0) {
    fstdistfunc_ = L2SqrII4x;
  } else {
    fstdistfunc_ = L2SqrII;
  }
  dim_ = dim;
