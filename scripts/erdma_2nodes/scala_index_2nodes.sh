...
REMOTE_NODES=("172.25.128.2")  # 远程节点列表，可扩展
...
million=100                                    # 数据规模（百万）
base_file=${dataset_path}/learn.${million}M.u8bin  # 使用learn文件作为base
...
# CMake配置 (eRDMA模式，2节点)
echo "  -> CMake配置..."
cmake -DUSE_ERDMA=ON -DMACHINE_NUM=2 ..
...
for REMOTE_NODE in "${REMOTE_NODES[@]}"; do
    ...
    # 确保远程目录存在
    ssh ${REMOTE_USER}@${REMOTE_NODE} "mkdir -p ${BUILD_DIR}/tests ${PROJECT_ROOT}/scripts/erdma_2nodes"
...
CONF_FILE="${SCRIPT_DIR}/ip_2nodes.conf"
...
echo ""
echo "[步骤4] 准备索引目录..."
index_save_dir=${index_save_path}/scalagraph/erdma_${million}M_2P
